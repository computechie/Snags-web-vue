<template>
  ...
  <div style="width: 100%" @contextmenu="showContextMenu($event)">
    ...
    >
      ...
      <Column
        field="date"
        header="Date Opened"
        style="min-width: 10rem; min-height: 43px"
      >
        <template #body="{ data }">
      ...
      >
        <template #body="{ data }">
          ...
          >
            <img
              v-if="data.caption == 'Kitchen Units'"
              src="/images/pin.png"
              style="height: 18px"
            />
            <img
  ...
  <div id="RightSidepanel" class="RightSidepanel" :class="panelClass">
    ...
    <div class="row">
      <div class="col text-center">
        <div
          v-if="snagDetailsData.status == 'Ready'"
          class="statusOpen statusBlock"
        >
          {{ snagDetailsData.status }}
        </div>
        <div
          v-if="snagDetailsData.status == 'Completed'"
          class="statusClosed statusBlock"
        >
          {{ snagDetailsData.status }}
        </div>
        <div
          v-if="snagDetailsData.status == 'Submitted'"
          class="statusActioned statusBlock"
        >
          {{ snagDetailsData.status }}
        </div>
        <div
          v-if="snagDetailsData.status == 'Reopen'"
          class="statusReturned statusBlock"
        >
          {{ snagDetailsData.status }}
        </div>
            ...
            <template #content="slotProps">
              <Card class="mt-3">
                ...
                <div class="historyNotes">
                  <textarea style="width: 250px; font-size: 75%">
Comments</textarea
                  >
                </div>
              </Card>
  ...
  methods: {
    ...
    async getSnags(all) {
      ...
      await axios
        .get(
          baseUrl +
            "/api/Snags/GetAllSnags/?pageSize=" +
            this.pageSize +
            "&pageNumber=" +
            pageA +
            addOnFilter,
    ...
    async getDocumentDetails() {
      ...
      await axios
        ...
        .catch(function (error) {
          ...
          this.InfoModalHeader = "Error!";
          this.InfoModalMessage =
            "An error occured while getting snag details!";
          this.isLoading = false;
        });
