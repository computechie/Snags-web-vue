<template>
  ...
  >
    <swiper-slide
      ...
      v-for="(tripImage, index) in tripImages"
      v-bind:key="tripImage.key"
    >
      <img
        @click="zoomImages(tripImage.key, index)"
        :src="
          baseUrl + methodName + parameter + tripImage.key + '&t=' + timestamp
        "
      ...
      <div class="deleteImageButton">
        <Button
          v-if="userType == 'ADMIN'"
          @click="sendBackIdForDelete(tripImage.key)"
          v-tooltip.left="'Delete photo'"
    ...
    <div :class="'zoomedImageHolder'">
      ...
      <div class="row">
        ...
        <div class="col-md-4 text-center">
          ...
          />
          <Button
            v-tooltip.bottom="'Download photo'"
            icon="pi pi-download"
            @click="downloadImage(zoomedImageId)"
            class="downloadImageButton"
          />
        </div>
        <div class="col-md-4 text-end">
          <Button
            v-if="userType == 'ADMIN'"
            @click="
              photoIdForDelete = zoomedImageId;
              displayDeletePhotoConfirm = true;
            "
            v-tooltip.top="{
              value: 'Delete photo',
              showDelay: 1000,
  ...
  >
    ...
    <div class="row" style="margin-top: 30px">
      ...
      <div class="col-md-6 text-end">
        <Button
          label="Yes, delete"
          @click="deletePhoto(photoIdForDelete)"
        ></Button>
      </div>
    </div>
  </Dialog>
...
export default {
  ...
  methods: {
    ...
    zoomImages(imageKey, index) {
      ...
      this.zoomedImageSource =
        this.baseUrl + this.methodName + this.parameter + imageKey;
      this.rotate = 0;
    ...
    onSlideChange() {
      var imageId = $(
        ".zoomedImageHolder .swiper .swiper-wrapper .swiper-slide-active img"
      ).attr("id");
...
.zoomedImageHolder .swiper,
.zoomedImageHolder .swiper-wrapper,
.zoomedImageHolder .swiper-wrapper {
